...
error NotOwner(address notOwnerAddress);

contract FundMe{

    uint256 public constant MINIMUM_USD = 5e18;     //Constant saves Gas since this Value will never change
    ...
    function fund() public payable{
        ...
        require(getConversionRate(msg.value) >= MINIMUM_USD, "Didnt send enough ETH");
    ...
    modifier onlyOwner() {  //ADDS SECURITY  GIVES ACCESS ONLY TO SPECIFIC OWNERS ADDRESS PERMISSION TO WITHDRAW FROM CONTRACT
        // require(msg.sender == i_owner, "Must be owner address !");
        if (msg.sender != i_owner) {
            revert NotOwner({notOwnerAddress: msg.sender});
        }
        _;
    }
